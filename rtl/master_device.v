// AHB master. Drives one transfer per trigger_transfer, retrying on RETRY or
// SPLIT until the slave completes it.
...
module master_device (
    ...
    localparam S_SAMPLE= 3'd5;   // read data valid this cycle
    ...
    always @(posedge HCLK or negedge HRESETn) begin
        ...
        if (!HRESETn) begin
            ...
            case (state)
                ...
                S_DATA: begin
                    if (HREADY) begin
                        ...
                        if (HRESP == `HRESP_OKAY) begin
                            ...
                            // A write is done -- the slave took HWDATA this
                            // cycle. A read is not: HRDATA only appears next
                            // cycle, so sampling here latches the previous
                            // transfer's byte.
                            if (HWRITE)
                                state <= S_DONE;
                            else
                ...
                // Read data is on the bus now -- unless the slave deferred in
                // the meantime, so re-check the response. Capturing
                // unconditionally would latch garbage and complete a transfer
                // the slave never performed.
                S_SAMPLE: begin
                    if (!HREADY) begin
                        // Slave is stretching the transfer; stay put.
                        state <= S_SAMPLE;
                    end
                    else if ((HRESP == `HRESP_RETRY) || (HRESP == `HRESP_SPLIT)) begin
                        HBUSREQ <= 1'b1;
                        HTRANS  <= HTRANS_IDLE;
                        state   <= S_WAIT;
                    end
                    else if (HRESP == `HRESP_OKAY) begin
                        HRDATA_OUT <= HRDATA;
                        state      <= S_DONE;
                    end
                    else begin
